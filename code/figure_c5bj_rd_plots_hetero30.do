...
*Low Mshare
ivregress 2sls covered runningvar2_norm_std c.femaleinstrument#c.runningvar2_norm_std (femalereservation  = femaleinstrument) [pw=weight] if abs(runningvar2_norm_std) <= bandwidth & post == 1 & covered201516 == 0 & muslim_share<.3, cluster(eleid)
...
gen mshare=0 if muslim_share<0.3
replace mshare= 1 if muslim_share>=.3
...
graph export "${path}\output\rd_plot_second10_msharelow_30threshold.eps", as(eps) preview(on) replace
...
graph export "${path}\output\rd_plot_second10_msharehigh_30threshold.eps", as(eps) preview(on) replace
